...
- [major] Expand Kafka events to cover all mutating changes on board/column/task/subtask — only Board/Column/Task Created/Deleted/Moved emit events today; Board/Column/Task updates and all Subtask CRUD (Subtask publishes nothing at all) are invisible to the activity log. See `.planning/todos/pending/2026-08-09-expand-kafka-events-to-cover-all-mutating-changes-on-board-c.md`.
