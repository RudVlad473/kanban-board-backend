...
- [security] Wire a `SessionAuthenticationStrategy` into the custom `AuthenticationController.signin` path — the concurrent-session ceiling (`maximumSessions(2)`) and session-fixation protection (`sessionFixation(newSession)`) are both configured but never applied, since no `SessionManagementFilter`/`SessionAuthenticationStrategy` runs on this app's direct `authenticationManager.authenticate()` signin path. A tripwire test (`SessionPersistenceE2ETest.ConcurrentSessionCeiling`) is designed to go red the day this is fixed. See `.planning/todos/pending/2026-08-02-wire-session-authentication-strategy-into-custom-signin.md`.
