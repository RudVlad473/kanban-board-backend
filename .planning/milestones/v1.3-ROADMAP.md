...
- [x] 10-04-PLAN.md — Gradle build-tooling integrity: `distributionSha256Sum` pin, `gradle/actions/wrapper-validation` ahead of every `./gradlew`, and dependency-verification metadata (folded todos only — a trust boundary CONTEXT.md defines as distinct from HARDEN-03, which plan 10-01 owns in full; no formal HARDEN-* ID)
