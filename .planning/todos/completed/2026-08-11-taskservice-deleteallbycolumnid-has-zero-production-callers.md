...
Resolved by quick task 260813-euo: option 1 (delete) was chosen, per the precedent of quick task
260802-q6n. Re-verified zero production callers by a fresh grep immediately before deleting.
Deleted `TaskService.deleteAllByColumnId`; the package-private `deleteAllByColumn(ColumnEntity)`
it delegated to is untouched and remains the live cascade path via `ColumnService`.
`TaskServiceTest.DeleteAllByColumnIdQueryCountTest` was deleted (re-verified, not assumed, that
`ColumnServiceTest.shouldCostSameQueryCount_regardlessOfTaskCountInColumn` already proves the same
no-scaling property, more strongly, through the live column-delete entry point).
`TaskServiceTest.DeleteAllByColumnIdTest.shouldDeleteAll_whenTasksExist` was migrated (not
dropped) to `ColumnServiceTest.DeleteByIdTest.shouldDeleteAllTasks_whenColumnHasTasks`, since that
assertion was not duplicated anywhere else — `fk_tasks_column`'s missing `ON DELETE CASCADE`
means a column delete that skipped its tasks would raise a foreign-key violation, so a clean
return is itself proof the cascade ran.
