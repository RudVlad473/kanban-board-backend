...
Resolved by quick task 260813-euo. The inner `@Size`'s `message` now names
`SUBTASK_TITLE_LENGTH_VALIDATION_MESSAGE`, exactly as suggested — diverging from this todo's own
"no meaningful RED-then-GREEN test" framing: a new `SubtaskTitleMessageTest` was added instead,
constructing a `jakarta.validation.Validator` directly and asserting both `SaveSubtaskRequestDTO`
and `UpdateSubtaskRequestDTO` render exactly one violation with `SubtaskTitle`'s own default
message on an over-long title. That assertion was itself falsified before being trusted:
temporarily removing `@ReportAsSingleViolation` made both new test cases fail (the inner `@Size`
text became visible), restoring it made them pass again — proving the test genuinely pins the
`@ReportAsSingleViolation` collapse rather than merely asserting a static fact.
