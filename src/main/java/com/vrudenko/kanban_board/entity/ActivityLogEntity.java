 ...
 * {@code uk_activity_log_event_id} constraint {@code
 * V6__change_activity_log_event_id_to_varchar.sql} carries; the migration, not this annotation, is
 * what production actually enforces (the real profile sets no {@code ddl-auto}). {@code eventId} is
 * a {@link String} holding a Base36 Snowflake-style id generated by {@code EventIdGenerator}
 * (GAP-07), not a random {@link java.util.UUID} — existing rows written before that change keep
 * their UUID string form, so the column holds a mix of both shapes, compared for equality only,
 * never parsed.
 */
@Entity
@Getter
