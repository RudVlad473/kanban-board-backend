...
- Handle **idempotent consumption**: give each event an `eventId` (originally specced here as a
  random UUID; switched in v1.2 Phase 6, GAP-07, to a time-ordered Base36 string generated by the
  project's existing `RandFlakeGenerator` — see `docs/ARCHITECTURE.md`); before inserting, check
